...
当项目有开发计划（不管是新项目还是二开），按以下层级创建文档：

**必建（有开发计划时）：**

| 文档 | 定位 | 内容 |
|------|------|------|
| `CLAUDE.md` | 项目说明书 | 技术栈、开发命令、约定、活跃文档清单 |
| `AGENTS.md` | 软链接 | → `CLAUDE.md` |
| `PLAN.md` | 开发计划 | 目标、阶段划分、任务拆分（相对稳定，不频繁改） |
| `STATUS.md` | 会话交接文档 | 当前进度、阻塞项、下次继续的入口（每次有实质进展时更新） |

**按需建（复杂项目）：**

| 文档 | 何时需要 |
|------|---------|
| `DESIGN.md` | 架构决策复杂到会让 CLAUDE.md 膨胀时 |
| `PLAN-xxx.md` | 某个阶段/模块的计划需要独立展开时 |
| `FORK.md` | 二开项目，需要记录与上游的差异和定制点 |

**不需要额外文档的情况：**
- vendor 项目（纯学习/参考）→ 只需 `CLAUDE.md`
- fork 只做小 PR 贡献 → 只需 `CLAUDE.md`，计划写在 CLAUDE.md 里即可

### 活跃文档清单（关键规则）

`CLAUDE.md` 中必须维护一个 `## 活跃文档` 小节，列出当前有效的文档及其用途。AI 进入项目时只读清单中的文档，不再猜测。

```markdown
## 活跃文档
- PLAN.md — 整体开发计划
- STATUS.md — 进度快照（上次更新: 2026-03-18）
- DESIGN.md — 架构设计
```
...
## 活跃文档
- PLAN.md — 开发计划
- STATUS.md — 进度快照（上次更新: yyyy-mm-dd）
```

> AI 读完 `CLAUDE.md` 后应能独立开发，无需额外询问基本信息。读完活跃文档清单后应知道还需要读哪些文件。
